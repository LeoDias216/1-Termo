...
janela.configure(bg="lightcoral")
...
def eficiencia_pecas():
    ...
    if pecas_produzidas == "" or pecas_defeituosas == "":
        ...
        proficiencia = pecas_defeituosas / pecas_produzidas
        if proficiencia > 0.05:
...
ent_pecas_defeituosas.grid(row=2, column=1, pady=10, padx=10)
...
def medida_peca():
    ...
    if medida == "":
        messagebox.showwarning("Aviso!", "Insira a medida da peça corretamente")
    else:
        if 9.8 <= medida <= 10.2:
            messagebox.showinfo("Dentro da tolerância", "A peça está boa")
        elif medida < 9.8:
            messagebox.showwarning("Aviso:", "Peça abaixo da tolerância")
        else:
            messagebox.showwarning("Aviso:", "Peça acima da tolerância ")

janela = tk.Tk()
janela.title("Validador de medida")
janela.geometry("700x500")
janela.configure(bg="olive")

lbl_medida_inserida = tk.Label(janela, text="Insira a medida da peça abaixo para validação:", font=("Arial", 14))
lbl_medida_inserida.grid(row=0, column=0, pady=20, padx=20)

lbl_medida = tk.Label(janela, text="Insira a medida:", font=("Arial", 10))
lbl_medida.grid(row=1, column=0, pady=20, padx=20)

ent_medida = tk.Entry(janela, font=("Arial", 12), width=20)
ent_medida.grid(row=1, column=1, pady=10, padx=10)

btn_verificar_medida = tk.Button(janela, text="Verificar", width=30, command=medida_peca)
btn_verificar_medida.grid(row=3, column=0, pady=10, padx=10)
...
def iniciar_prensa():
    for contagem in range(10, 0, -1):
        messagebox.showinfo("Aviso", f"A prensa vai ativar em {contagem} segundos")
    messagebox.showwarning("AVISO", "A prensa foi ativada")

janela = tk.Tk()
janela.title("Contagem da Prensa")
janela.geometry("600x500")
janela.configure(bg="snow")

lbl_contagem_prensa = tk.Label(janela, text="Inicie a contagem da prensa para seu funcionamento", font=("Arial", 14))
lbl_contagem_prensa.grid(row=0, column=0, padx=20, pady=20)

btn_contagem_prensa = tk.Button(janela, text="Iniciar", width=30, command=iniciar_prensa)
btn_contagem_prensa.grid(row=1, column=0, padx=10, pady=10)
